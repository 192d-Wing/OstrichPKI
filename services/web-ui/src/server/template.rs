...
/// Serve the React (preview) SPA index with CSP-nonce + runtime-config injection.
///
/// Temporary `/next` preview of the React/shadcn frontend (P1 of the Yew→React
/// migration). The live Yew app at `/` is unaffected. The Vite-built index.html
/// loads its module script + stylesheet from same-origin `/static/assets/*`,
/// already permitted by the CSP `'self'` source, so ONLY the injected inline
/// config script needs the per-request nonce.
///
/// COMPLIANCE MAPPING:
...
pub async fn serve_next_index(
    State(state): State<AppState>,
    Extension(nonce): Extension<CspNonce>,
) -> Response {
    // The Vite build's index.html is copied next to the static assets at build
    // time (see services/web-ui/Dockerfile).
    let path = std::path::Path::new(&state.config.static_files.directory).join("next-index.html");
    let template = match std::fs::read_to_string(&path) {
        Ok(t) => t,
        Err(e) => {
            tracing::error!(error = %e, path = %path.display(), "React preview index not found");
            return (
                StatusCode::NOT_FOUND,
                "React preview not built for this image",
            )
                .into_response();
        }
    };

    // basename "/next" mounts the SPA under the preview path; the rest mirrors
    // the Yew ClientConfig so the React app reads the same runtime config.
    ...
    let config_json = match serde_json::to_string(&config) {
        Ok(j) => j,
        Err(e) => {
            tracing::error!(error = %e, "Failed to serialize preview config");
            return (StatusCode::INTERNAL_SERVER_ERROR, "Configuration error").into_response();
        }
    };
